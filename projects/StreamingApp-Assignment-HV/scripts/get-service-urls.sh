...
echo "auth      : http://${AUTH:-<pending>}:4001"
echo "streaming : http://${STREAM:-<pending>}:4002"
echo "admin     : http://${ADMIN:-<pending>}:4003"
echo "chat      : http://${CHAT:-<pending>}:4004"
echo ""
echo "# Paste these before running build-and-push-ecr.sh (Run 2),"
echo "# or copy the values into the Jenkinsfile FE_* variables:"
echo "export FE_AUTH=http://${AUTH}:4001/api"
echo "export FE_STREAM=http://${STREAM}:4002/api"
echo "export FE_STREAM_PUB=http://${STREAM}:4002"
echo "export FE_ADMIN=http://${ADMIN}:4003/api/admin"
echo "export FE_CHAT=http://${CHAT}:4004/api/chat"
echo "export FE_CHAT_SOCK=http://${CHAT}:4004"
echo ""
